"""
This script converts the input image sequence into a mov file (codec prores422)
"""
...
class ImgSequenceToProres422():
    ...
    def output_path(self) -> Path:
        ...
        path = path + ".mov"
        path = Path(path)
        path = path.parent.joinpath("Prores422", path.name)
        return path
    ...
    def ffmpeg_command(self) -> str:
        ...
        command += ["-profile:v", "2"]  # Profile 422
        command += ["-pix_fmt", "yuv422p10le"]  # 10 bits 422 Subsampling
        command += ["-vendor", "apl0"]
...
if __name__ == "__main__":
    ...
    for input_path in sys.argv[1:]:
        ...
        if input_path.suffix.lower() in [".png", ".jpeg", ".jpg", ".exr", ".tiff", ".tif", ".tga"]:
            transcoder = ImgSequenceToProres422(input_path=input_path)
        else:
            transcoder = VideoToProres422(input_path=input_path)
